...
def install() -> None:
    ...
    if current_exe.is_file() and current_exe.suffix.lower() == ".exe":
        ...
        # 如果 exe 是从源码跑的则 __main__ 与打包 exe 不一样,这里直接复用自身
    else:
    ...
    for parent in REG_LOCATIONS:
        ...
        _reg_write(winreg.HKEY_CURRENT_USER, menu_key, "Icon", f'"{dst_exe}"')
    ...
    _reg_write(winreg.HKEY_CURRENT_USER, APP_UNINSTALL_KEY, "DisplayIcon", str(dst_exe))
